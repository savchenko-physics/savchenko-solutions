...
function summarize(info) {
    ...
    const duration = Number((info.format && info.format.duration) || (video || audio).duration || 0) || 0;
    if (!video) {
        return { formats, duration, video: null, audio: { codec: audio.codec_name, channels: Number(audio.channels) || 0 }, extraStreams: false };
    }
...
// Audio that every browser plays: MP3 in an mp3, AAC in an m4a, PCM in a wav.
const PLAYABLE_AUDIO_FILES = { mp3: ['mp3'], m4a: ['aac'], wav: ['pcm_s16le', 'pcm_u8', 'pcm_s24le', 'pcm_f32le'] };

/** 'none' when the file already plays everywhere, 'remux' when only the container is wrong,
 *  'transcode' (H.264 mp4) or 'audio' (AAC m4a) otherwise. `ext` is the upload's extension,
 *  lower-case, without the dot. */
function conversionNeeded(info, ext) {
    if (!info) return 'transcode';
    if (!info.video) {
        const ok = PLAYABLE_AUDIO_FILES[ext];
        return ok && info.audio && ok.includes(info.audio.codec) ? 'none' : 'audio';
    }
    const v = info.video;
...
function ffmpegArgs(mode, src, dst) {
    ...
    if (mode === 'audio') {
        return ['-nostdin', '-hide_banner', '-loglevel', 'error', '-y', '-i', src, '-vn', '-sn', '-dn', '-map_metadata', '-1',
...
async function convert(src, dst, mode, runner = runFfmpeg) {
    const modes = mode === 'remux' ? ['remux', 'transcode'] : [mode === 'audio' ? 'audio' : 'transcode'];
    let lastErr = null;
...
/** Name of the playable file for an upload: <base>-web.mp4 (or -web.m4a for audio) next to it. */
function outputPathFor(src, mode) {
    const dir = path.dirname(src);
    const base = path.basename(src).replace(/\.[A-Za-z0-9]+$/, '');
    return path.join(dir, `${base}-web.${mode === 'audio' ? 'm4a' : 'mp4'}`);
}
