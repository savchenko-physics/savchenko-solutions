...
function currentSolvedCount() {
    // Required here, not at the top: the tests load this file without the site's posts.
    return require('../unsolved').getSolutionProgressStats('en').then((stats) => stats.solvedProblems);
}

/* The gate for one person right now, with the notice to show, or null when there is nothing to say. */
async function founderYearFor(userId, lang) {
    let solved;
    try {
        solved = await currentSolvedCount();
    } catch (err) {
