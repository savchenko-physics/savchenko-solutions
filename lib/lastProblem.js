...
/* What a person has earned in the market: their trading profit plus the bounties for problems they
 * solved (lastProblem.js payBounties). The leaderboard ranks by this since 2026-09-18, when a board of
 * trading profit alone had the demon on top and all five traders below zero, while the people who
 * had solved fourteen problems between them did not appear on it at all. Quanta spent on reactions
 * are not subtracted: spending what you won is not losing it. */
function earnedOf({ spent = 0, received = 0, interest = 0, value = 0, bounty = 0 } = {}) {
    return profitOf({ spent, received, interest, value }) + (Number(bounty) || 0);
}

/* The ranking: most earned first, then the better trader, then by name. The demon is never in it:
 * it trades by the model with the house's money and is shown apart, as the benchmark. */
function rankBoard(entries) {
    return (entries || [])
        .filter((e) => e && !e.demon)
        .sort((a, b) => (b.earned - a.earned) || ((b.profit || 0) - (a.profit || 0)) || String(a.username).localeCompare(String(b.username)));
}
