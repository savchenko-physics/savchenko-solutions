...
// No parallel query on the app's connections. RDS's small instance takes ~20 ms to launch the
// two workers the planner asks for on a scan it estimates at a few hundred rows (a problem's
// view rows: 0.4 ms of work inside 21 ms, measured 2026-09-19), and nothing a page runs is
// large enough to gain from them; the few heavy aggregates are cached (lib/swr.js).
pool.on('connect', (client) => {
    client.query('SET max_parallel_workers_per_gather = 0').catch((err) => {
        console.error('pg pool: could not set session parameters:', err.message);
    });
});
