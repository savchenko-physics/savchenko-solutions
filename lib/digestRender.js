...
//   - padding is small, 16px at the edges and 12px inside a card. Mail is read in a narrow
//     column, often on a phone, and generous whitespace there reads as emptiness.
//
// The owner's punctuation rule (the one tests/page-titles.test.js keeps for page titles) holds
// for every word this file writes: no interpunct, no em or en dash, no colon, no semicolon.
// Words and line breaks do the separating instead. What a person typed is quoted as they typed
// it: an excerpt is somebody else's sentence, and editing its punctuation misquotes them.
//
// The data comes from digest.js; this file only formats it and escapes it.

const { ruPlural } = require('./ruPlural');
const { colour, SERIES } = require('./siteColours');

// Not one hex in this file: every colour is a token out of css/design-system.css, so the email
// changes when the site does (lib/siteColours.js).
const C = {
    navy: colour('navy'),
    text: colour('text'),
    secondary: colour('text-secondary'),
    link: colour('link'),
    band: colour('link'),                  // the site's link blue, used as a field
    bandSoft: colour('on-navy-link'),      // labels and links on a dark field
    rule: colour('rule'),
    surface: colour('surface'),
    surfaceAlt: colour('surface-alt'),
    page: colour('surface-hover'),         // the grey the site uses behind cards
    onNavy: colour('on-navy'),
    goodInk: colour('success-strong'),
    goodFill: colour('success-soft'),
};
// The four data-series colours (js/palettes.js), so an avatar circle is always a colour the
// site already owns and always the same one for the same person.
const AVATAR_COLOURS = SERIES;
...
/** "с 9 по 16 сентября" / "9 to 16 September", in words because a dash is not allowed here. */
function periodLabel(fromISO, toISO, lang) {
    ...
    if (lang === 'ru') {
        return sameMonth
            ? `с ${from.getUTCDate()} по ${to.getUTCDate()} ${months[to.getUTCMonth()]}`
            : `с ${from.getUTCDate()} ${months[from.getUTCMonth()]} по ${to.getUTCDate()} ${months[to.getUTCMonth()]}`;
    }
    return sameMonth
        ? `${from.getUTCDate()} to ${to.getUTCDate()} ${months[to.getUTCMonth()]}`
        : `${from.getUTCDate()} ${months[from.getUTCMonth()]} to ${to.getUTCDate()} ${months[to.getUTCMonth()]}`;
}
...
const COPY = {
    ru: {
        ...
        youTitle: 'Про вас',
        replied: (who, problem) => `${who} ответил вам в задаче ${problem}`,
        ...
        settingsInline: 'настройки писем',
        unsubscribeInline: 'отписаться',
        // The two arguments are links this file built and escaped; the words around them are
        // its own. A sentence, because a bar between two links is a separator too.
        footerAction: (settingsLink, unsubscribeLink) => `Можно изменить ${settingsLink} или ${unsubscribeLink}.`,
    ...
    en: {
        ...
        youTitle: 'About you',
        replied: (who, problem) => `${who} replied to you on problem ${problem}`,
        ...
        settingsInline: 'your email settings',
        unsubscribeInline: 'unsubscribe',
        footerAction: (settingsLink, unsubscribeLink) => `You can change ${settingsLink} or ${unsubscribeLink}.`,
...
/**
 * Somebody's own picture, round, with their initial on a coloured circle underneath. The letter
 * is the image's alt text, so a client that blocks images (most of them, until the reader says
 * otherwise) or cannot read WebP (Outlook on Windows) shows the circle and the letter instead
 * of a broken frame. 41 of the site's accounts have a picture; the rest are the circle.
 */
const avatarCell = (name, picture = null, size = 32) => {
    const circle = `width:${size}px;height:${size}px;background:${avatarColour(name)};border-radius:${size}px;font-family:${FONT};font-size:${Math.round(size * 0.42)}px;font-weight:700;color:${C.onNavy};`;
    const inner = picture
        ? `<img src="${escapeHtml(picture)}" width="${size}" height="${size}" alt="${escapeHtml(initial(name))}" style="display:block;width:${size}px;height:${size}px;border-radius:${size}px;border:0;color:${C.onNavy};font-family:${FONT};font-size:${Math.round(size * 0.42)}px;font-weight:700;text-align:center;line-height:${size}px;">`
        : escapeHtml(initial(name));
    return `
              <td width="${size + 8}" valign="top" style="width:${size + 8}px;">
                <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr>
                  <td width="${size}" height="${size}" align="center" valign="middle" style="${circle}">${inner}</td>
                </tr></table>
              </td>`;
};

function personalItem(item, t) {
    const line = item.kind === 'reply' ? t.replied(item.author, item.problem) : t.commented(item.author, item.problem);
    return `
          <tr><td style="padding:10px 16px 0;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
${avatarCell(item.author, item.authorPicture)}
...
${avatarCell(d.lastAuthor, d.lastAuthorPicture, 28)}
...
${counterCell(d.counters.members, t.counters.members(d.counters.members))}
    ...
    if (hasPersonal) {
        ...
        if (followers.length > 0) {
          ...
          <tr><td style="padding:10px 16px 0;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
${avatarCell(names[0], followers[0].picture)}
        ...
        if (likes > 0) {
          ...
          <tr><td style="padding:10px 16px 0;">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr>
              <td style="background:${C.goodFill};border-radius:4px;padding:6px 10px;font-family:${FONT};font-size:13px;font-weight:600;color:${C.goodInk};">${escapeHtml(t.likes(likes))}</td>
            </tr></table>
          </td></tr>`);
        }
    ...
    if ((d.updates || []).length > 0) {
        ...
        parts.push(`
          <tr><td style="padding:6px 16px 0;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
${d.updates.slice(0, 8).map((u) => listRow(t.problem(u.problem), t.byIn(u.authors.join(', '), t.langsLabel(u.languages || [u.language])), u.url)).join('')}
            </table>
          </td></tr>`);
    }
    ...
    parts.push(`
          ...
          <tr><td style="padding:16px;">
            ...
            <div style="font-family:${FONT};font-size:12px;line-height:1.6;color:${C.secondary};padding-top:10px;text-align:center;">
              ...
              ${t.footerAction(
                  `<a href="${escapeHtml(d.settingsUrl)}" style="color:${C.link};text-decoration:underline;">${escapeHtml(t.settingsInline)}</a>`,
                  `<a href="${escapeHtml(d.unsubscribeUrl)}" style="color:${C.link};text-decoration:underline;">${escapeHtml(t.unsubscribeInline)}</a>`
              )}
            </div>
          </td></tr>`);
...
${parts.join('')}
    ...
    if (hasPersonal) {
        ...
        for (const item of personal.slice(0, 6)) {
            lines.push(`- ${item.kind === 'reply' ? t.replied(item.author, item.problem) : t.commented(item.author, item.problem)}`, `  ${excerpt(item.excerpt, 100)}`, `  ${item.url}`);
        }
    ...
    if ((d.updates || []).length) {
        ...
        for (const u of d.updates.slice(0, 8)) lines.push(`- ${t.problem(u.problem)}, ${t.byIn(u.authors.join(', '), t.langsLabel(u.languages || [u.language]))}`, `  ${u.url}`);
        lines.push('');
    }
    if ((d.wanted || []).length) {
