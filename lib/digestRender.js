...
${d.updates.slice(0, 8).map((u) => listRow(t.problem(u.problem), `${u.authors.join(', ')} · ${t.langLabel(u.language)}`, u.url)).join('')}
    ...
    if ((d.wanted || []).length > 0) {
        parts.push(sectionTitle(t.wantedTitle));
        parts.push(`
          <tr><td style="padding:8px 24px 0;font-family:${SANS};font-size:13px;color:${C.secondary};">${escapeHtml(t.wantedNote)}</td></tr>
          <tr><td style="padding:8px 24px 0;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
...
${parts.join('')}
    ...
    if ((d.wanted || []).length) {
        lines.push(t.wantedTitle.toUpperCase());
