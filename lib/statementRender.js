...
function sourceText(row, name, lang) {
    const own = breakSubItems(repairStrayDollar(row.statement_tex));
    if (!row.needs_review) return { tex: own, needsReview: row.needs_review };
    const fromPost = postStatement(name, lang);
    if (fromPost && /\$/.test(fromPost)) return { tex: breakSubItems(fromPost), needsReview: false };
    return { tex: own, needsReview: row.needs_review };
}
...
module.exports = { getStatement, getStatements, repairStrayDollar, breakSubItems, CACHE_MAX, CACHE_TTL };
