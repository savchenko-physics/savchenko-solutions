...
function sourceText(row, name, lang) {
    if (!row.needs_review) return { tex: row.statement_tex, needsReview: row.needs_review };
    const fromPost = postStatement(name, lang);
    if (fromPost && /\$/.test(fromPost)) return { tex: fromPost, needsReview: false };
    return { tex: row.statement_tex, needsReview: row.needs_review };
}
