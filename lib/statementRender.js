/**
 * statementRender.js — render a problem statement to display-ready HTML, with a shared
 * bounded cache.
 *
 * Extracted from index.js's GET /api/problem/:name/statement so the problem finder's batch
 * endpoint reuses the same cache rather than keeping a second copy of the same SVG. Statement
 * SVG runs to tens of kilobytes and there are 4,046 of them, so two independent caches would
 * be real duplicated memory on a process that is already large.
 *
 * The cache is bounded and has a TTL rather than being permanent: statements do change under
 * a running server when scripts/repair-ru-latex.js re-typesets a row, and a cache that never
 * expires would serve the flattened version until the next restart.
 */

const fs = require('fs');
const path = require('path');
const { parseMarkdown, transformImageMarkdown } = require('../utils');
const { renderMathInHtml } = require('../mathRender');
const { linkProblemRefs } = require('./problemRefs');
const { postStatement } = require('./postStatement');

// The figures attached through `figures` are the book's own drawings (scripts/book3/):
// SVG traced from the 300 dpi bitmaps (scripts/book3/vectorize.py), whose width/height
// attributes already carry the display size — 0.42 of the bitmap's pixel width, about
// 1.25x the printed size, clamped to 140–560px. A PNG (the pre-vector files) gets the same
// rule applied to its IHDR width. Sizes are read once per path and cached; a file that
// cannot be read gets no width attribute and falls back to CSS max-width.
const dimsCache = new Map();
function displaySize(src) {
    if (dimsCache.has(src)) return dimsCache.get(src);
    let size = null;
    try {
        const fd = fs.openSync(path.join(__dirname, '..', src.replace(/^\//, '')), 'r');
        const head = Buffer.alloc(600);
        const n = fs.readSync(fd, head, 0, 600, 0);
        fs.closeSync(fd);
        if (head.toString('ascii', 1, 4) === 'PNG') {
            const pw = head.readUInt32BE(16);
            const ph = head.readUInt32BE(20);
            const w = Math.max(140, Math.min(560, Math.round(pw * 0.42)));
            size = { w, h: Math.round(ph * w / pw) };
        } else {
            const text = head.toString('utf8', 0, n);
            const w = text.match(/<svg[^>]*\swidth="(\d+)"/);
            const h = text.match(/<svg[^>]*\sheight="(\d+)"/);
            if (w) size = { w: Number(w[1]), h: h ? Number(h[1]) : null };
        }
    } catch { /* missing or unreadable — leave unsized */ }
    dimsCache.set(src, size);
    return size;
}
// Both dimensions, so the figure's box is reserved before the file arrives: with the width
// alone the statement below a figure, and a preview card holding one, moved when it loaded
// (2026-09-20).
function figureHtml(src, name, lang) {
    const size = displaySize(src);
    const alt = lang === 'ru' ? `К задаче ${name}` : `Figure for problem ${name}`;
    const dims = size ? ` width="${size.w}"${size.h ? ` height="${size.h}"` : ''}` : '';
    return `<figure class="statement-figure"><img src="${src}" alt="${alt}" loading="lazy"${dims} /></figure>`;
}

// A statement whose dollars do not pair up is one the builder left a stray opening dollar in
// front of ("а.$ Из…", the closing dollar of "$1.1.9. а.$" after its number was stripped), and
// marked set everything up to the next dollar as maths: a sentence in italics with its spaces
// gone (/ru/1.1.9's preview, 2026-09-20). The fragment is dropped, the letter kept. Only when
// the count is odd: "а. $x$" is fine as it is.
const STRAY_OPENING = /^\s*(?:([а-яa-z])\s*\.\s*|\*\s*\.?\s*|\^\s*\{?\s*[*∗]\s*\}?\s*\.?\s*)\$\s*/i;
function repairStrayDollar(tex) {
    const text = String(tex || '');
    const dollars = (text.match(/\$/g) || []).length;
    if (dollars % 2 === 0) return text;
    return text.replace(STRAY_OPENING, (m, letter) => (letter ? `${letter}. ` : ''));
}

// A single line break inside a paragraph is a space. The posts wrap their lines as the book
// does (146 Russian statements, CRLF and all), and marked here turns every newline into <br>,
// so a statement showed "в любой момент<br>времени равна…" with a word alone on a line
// (10.2.1, 10.2.7, 10.2.9 in the previews, 2026-09-20). A blank line still ends a paragraph;
// a line that opens a block (display maths, an image, a list, a table, a heading, a tag) or
// one ending in markdown's two-space hard break keeps its break.
const BLOCK_START = /^\s*(?:\$\$|!\[|[-*+]\s|\d+\.\s|\||#|<)/;
function joinSoftBreaks(tex) {
    const lines = String(tex || '').replace(/\r\n?/g, '\n').split('\n');
    const out = [];
    for (const line of lines) {
        const prev = out.length ? out[out.length - 1] : null;
        const joinable = prev !== null && prev.trim() !== '' && line.trim() !== ''
            && !BLOCK_START.test(line) && !BLOCK_START.test(prev) && !/ {2,}$/.test(prev);
        if (joinable) out[out.length - 1] = `${prev.replace(/\s+$/, '')} ${line.replace(/^\s+/, '')}`;
        else out.push(line);
    }
    return out.join('\n');
}

// Sub-items of a statement start their own paragraph. Twenty-six statements have "а. … б. …"
// on one line (the text as the book sets it, or as it was typed), and the preview card showed
// "…со скоростью βc? б. Решите задачу…" as one run (14.2.18, 2026-09-20). A letter б–д (b–e)
// with a dot, after a sentence end and before a capital, opens a paragraph; "и т. д. Чем" does
// not, because the "т." before it is a single letter, not a sentence.
const SUB_ITEM = /(\S{2,}[.?!)])[ \t]+([б-дb-e])\.[ \t]+(?=[А-ЯЁA-Z])/g;
function breakSubItems(tex) {
    return String(tex || '').replace(SUB_ITEM, '$1\n\n$2. ');
}

// A statement still flattened from the scan (needs_review) is shown from the solution post's
// own typeset statement when there is one (lib/postStatement.js): the contributor set the
// maths the scan lost. Only when the post's section has maths in it; otherwise the row.
function sourceText(row, name, lang) {
    const clean = (tex) => breakSubItems(joinSoftBreaks(repairStrayDollar(tex)));
    if (!row.needs_review) return { tex: clean(row.statement_tex), needsReview: row.needs_review };
    const fromPost = postStatement(name, lang);
    if (fromPost && /\$/.test(fromPost)) return { tex: clean(fromPost), needsReview: false };
    return { tex: clean(row.statement_tex), needsReview: row.needs_review };
}

const CACHE_MAX = 600;
const CACHE_TTL = 30 * 60 * 1000;
const cache = new Map();

/**
 * @returns {Promise<{name,lang,html,figures,starred,needsReview}|null>} null when there is
 * no statement on record for that problem/language.
 */
async function getStatement(pool, name, lang) {
    const key = `${name}:${lang}`;
    const hit = cache.get(key);
    if (hit && Date.now() - hit.at < CACHE_TTL) return hit.payload;

    const { rows } = await pool.query(
        `SELECT statement_tex, figures, starred, source, needs_review
           FROM problem_statements WHERE problem_name = $1 AND lang = $2`,
        [name, lang]
    );
    if (!rows.length) return null;
    const row = rows[0];

    // Order matters and matches post.js — transformImageMarkdown consumes the site's
    // "![alt|WxH,scale%](…)" syntax, so it has to see the raw markdown; run it after
    // marked() and the dimensions end up as literal text in the alt attribute. It emits
    // "../../img/…", which only resolves from a two-segment URL like /en/1.1.1, so the
    // paths are made absolute for callers mounted elsewhere.
    const source = sourceText(row, name, lang);
    let html = parseMarkdown(transformImageMarkdown(source.tex))
        .replace(/(src|srcset)="\.\.\/\.\.\/img\//g, '$1="/img/');
    html = linkProblemRefs(renderMathInHtml(html), lang, { self: name });

    // Statements recovered from the book carry no image markdown, but many do have a figure
    // on disk that nothing references. Attach it when the text did not already bring one.
    if (!/<img/i.test(html) && row.figures.length) {
        html += row.figures.map((src) => figureHtml(src, name, lang)).join('');
    }

    const payload = {
        name,
        lang,
        html,
        figures: row.figures,
        starred: row.starred,
        // Statements recovered from the printed book still have flattened maths; the UI
        // says so rather than presenting them as clean.
        needsReview: source.needsReview,
    };

    // Map preserves insertion order, so the oldest key is the first one it yields.
    if (cache.size >= CACHE_MAX) cache.delete(cache.keys().next().value);
    cache.set(key, { at: Date.now(), payload });
    return payload;
}

/**
 * Same as getStatement but for many problems at once, in a single round trip to Postgres.
 *
 * Calling getStatement in a loop would issue one query per problem (a classic N+1) — for a
 * 20-card page that is 20 sequential DB round trips, which dominates the response on a cold
 * cache. This fetches every uncached row with one `= ANY(...)` query and renders from that.
 *
 * @returns {Promise<Object<string,string>>} problem name -> HTML, omitting any with no
 * statement on record.
 */
async function getStatements(pool, names, lang) {
    const out = {};
    const missing = [];

    for (const name of names) {
        const hit = cache.get(`${name}:${lang}`);
        if (hit && Date.now() - hit.at < CACHE_TTL) out[name] = hit.payload.html;
        else missing.push(name);
    }
    if (!missing.length) return out;

    const { rows } = await pool.query(
        `SELECT problem_name, statement_tex, figures, starred, needs_review
           FROM problem_statements WHERE problem_name = ANY($1) AND lang = $2`,
        [missing, lang]
    );

    for (const row of rows) {
        const source = sourceText(row, row.problem_name, lang);
        let html = parseMarkdown(transformImageMarkdown(source.tex))
            .replace(/(src|srcset)="\.\.\/\.\.\/img\//g, '$1="/img/');
        html = linkProblemRefs(renderMathInHtml(html), lang, { self: row.problem_name });
        if (!/<img/i.test(html) && row.figures.length) {
            html += row.figures.map((src) => figureHtml(src, row.problem_name, lang)).join('');
        }
        const payload = {
            name: row.problem_name,
            lang,
            html,
            figures: row.figures,
            starred: row.starred,
            needsReview: source.needsReview,
        };
        if (cache.size >= CACHE_MAX) cache.delete(cache.keys().next().value);
        cache.set(`${row.problem_name}:${lang}`, { at: Date.now(), payload });
        out[row.problem_name] = html;
    }
    return out;
}

module.exports = { getStatement, getStatements, repairStrayDollar, breakSubItems, joinSoftBreaks, CACHE_MAX, CACHE_TTL };
